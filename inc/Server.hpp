...
class Webserv {
	...
	public:
		...
		int	Send(int clientfd, std::string buffer, char **env);
		...
		int homemade_post(Request &request);
		std::string getUniqueFilename(const std::string& baseName, const std::string& extension, const std::string& directory);
